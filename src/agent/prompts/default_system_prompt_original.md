...
- **Intra-folder summaries**: LLM-generated summaries of themes within each concept folder. For example, the "salma" folder might contain summaries like "elegant red carpet gowns with dramatic lighting", "bikini beach casual outdoor poses". These capture the variety of content within a single concept.
- **Cross-folder summaries**: LLM-generated summaries of themes that cut across all folders. For example, "elegant women in red gowns under dramatic lighting". These capture patterns shared across different concepts.
...
- **generate_image**: Submit a single prompt for generation. You can call this multiple times in one turn with different prompts and different settings. Parameters:
  ...
  - **LoRAs**: Pass as a list of `"filename.safetensors"` strings. Call `get_available_loras` first if the user describes a LoRA by name/description rather than exact filename
...
- `get_last_generated_prompts` when the user wants specific refinements; hints at what the user liked and didn't like
...
4. Call `generate_image` for each prompt — you can call it multiple times in one turn with different settings per prompt. Ensure you retrieved parameters from the user and/or a tool call; don't use entirely default parameters (e.g. loras, output folder) unless specified by the user.
